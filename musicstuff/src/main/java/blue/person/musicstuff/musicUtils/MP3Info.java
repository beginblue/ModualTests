...
//    }
     ...
     */
    MP3Info(String path) throws IOException{
        ...
        if(!"TAG".equalsIgnoreCase(new String(buf,0,3))){//信息格式是否正确
            //throw new IOException("MP3标签信息数据格式不正确!"+new String(buf,0,3));
            mMusic = new Music();
            mMusic.setUri(path);
            mMusic.setTitle(path.substring(path.lastIndexOf('/')+1));
        }else {
            mMusic= generateMusic();
        }
    ...
    public  String getSongName(){
        try {
            ...
            int endIndex = path.lastIndexOf(".mp3");
            String fullName = path.substring(beginIndex,endIndex);
            return fullName;
        }
    }
    ...
    private Music generateMusic(){
        Music music = new Music();
        music.setUri(path);
        music.setYear(getYear());
        music.setFileName(path.substring(path.lastIndexOf('/')));
        music.setAlbum(getAlbum());
        music.setArtist(getArtist());
        music.setTitle(getSongName());
        return music;
    }
